...
json_file_path = '0720-0811 json/week 8 07-13'
                ...
                for oklink_txn in response_data['data']:
                    for json_txn in json_data:
                        if json_txn['Txn Hash'] == oklink_txn['txid']:
                            ...
                            json_txn['Txn Fees'] = f"{float(json_txn['Txn Fees']):.8f}"
                            ...
                            json_txn['Txn Fee Ratio'] = f"{((float(json_txn['Txn Fees'])/float(json_txn['Txn Output Amount']))*100):.8f}" if float(json_txn['Txn Output Amount']) != 0 else "0"
